...
//    printf("%f \n", duration);
    ...
    checkCudaErrors(cudaBindTexture2D(0, max_disp_cost_tex, max_disp_cost.ptr,
     ...
     // return first
     Mat resultWithoutOpt(_rows, _cols, CV_32F);
     checkCudaErrors(cudaMemcpy2D((float*)resultWithoutOpt.data, width*sizeof(float), min_disp.ptr, min_disp.pitch, width*sizeof(float), height, cudaMemcpyDeviceToHost));
//     return resultWithoutOpt;
